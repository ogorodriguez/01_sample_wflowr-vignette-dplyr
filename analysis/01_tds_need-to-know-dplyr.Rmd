...
`summarise()` has now been expanded to return vectors from multiiple inputs.
Also it can return now dataframes.  
...
`across()` was introduced to allow more powerful summarise actions together with the
`tidyselect()` verbs.  `across()` operates across multiple columns and applying dplyr
functions.  It represents a time-saving solution removing the need to use the *_at(), or
*_if() type of functions of previous dplyr versions.

For example to get the mean of various numercial columns of `mtcars_test`
...
`across()` is a selecting function.  We can identify which functions to apply, 
and then select the columns they will be applied to.
.
```{r selecting columns and functions to apply to}
dplyr::storms %>% 
...
```
Using `glue()` syntax to add the name of the column and the function applied to the
names fo the new columns.
